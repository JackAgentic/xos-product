...
export function ActivitiesCard({ activities, changeTab, className }: ActivitiesCardProps) {
  return (
    <div className={`bg-white rounded-sm border border-gray-200 p-4 h-full flex flex-col ${className ?? ''}`} data-ai-section="Recent Activities">
      ...
      <div className="flex-1 overflow-y-auto min-h-0">
        <div className="space-y-4">
          ...
          {activities.map((activity, idx) => (
            <div key={activity.id} className="relative" data-ai-field="activityRecord" data-ai-label="Activity Record" data-ai-entity-id={activity.id}>
              {idx !== activities.length - 1 && (
                <div className="absolute left-4 top-10 bottom-0 w-px bg-gray-200"></div>
              )}
              <div className="flex gap-3">
                <div className={`w-8 h-8 rounded flex items-center justify-center flex-shrink-0 ${activity.status === 'upcoming'
                  ? 'bg-stone-200/50'
                  : 'bg-gray-100'
                  }`}>
